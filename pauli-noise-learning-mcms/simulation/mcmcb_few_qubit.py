            ...
            for cpd in range(1):
                for nrest in unmeas_widths:
                    for nmeas in meas_widths:
                        ...
                        num_errors = (2**nmeas-1)*(4**nrest-1)
                        num_errors_during = 4**nrest-1
                        ...
                        probs_post, errors_post, probs_pre, errors_pre, probs_during, errors_during = generate_error_model(qubits, nmeas, nrest, num_errors, num_errors_during, noise_strengths)
                        ...
                        if rank==0:
                            ...
                            prog = False
                        est, est_std, evals_by_p, stds, energies = estimate_process_fidelity(10, nrest, nmeas, full_noisy_layer,num_randomizations, depths, prep_err, meas_err, show_progress=prog, nshots=10000, all_paulis=True)                   
                        ...
                        with open(f'results/stim/{nmeas}_{nrest}_qubit_mcmcb_run_{idx}_full_estimate.pkl', 'wb+') as f:
